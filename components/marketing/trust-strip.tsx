 ...
 *
 * The section fades in with a subtle slide-up via `ScrollReveal` once it
 * enters the viewport. Gated on `prefers-reduced-motion` end-to-end via
 * the `ScrollReveal` wrapper and the `Marquee` primitive.
 */
export function TrustStrip() {
  ...
  const items = [
    ...
    { Icon: Monitor, label: t("items.offline") },
    { Icon: ShieldCheck, label: t("items.gst") },
    { Icon: Barcode, label: t("items.barcode") },
    { Icon: CreditCard, label: t("items.payments") },
    { Icon: CalendarDays, label: t("items.trial") },
  ];
  ...
  return (
    <ScrollReveal direction="up" amount={0.1}>
      ...
      >
        ...
        <Marquee className="mask-fade-x" speedSeconds={50}>
